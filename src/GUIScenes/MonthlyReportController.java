...
//    );
	...
	public void addDataToTable() {
		...
		if (dateForData.getValue() != null && !txtfieldFinePerMinute.getText().isEmpty()
			...
			if (completeCircFineDay.getFill() == Color.GREEN && completeCircFineMinute.getFill() == Color.GREEN) {
				...
				series.getData().add(new XYChart.Data<>("January",
						db.Membertable.new_member(1, year, s.withMonth(1).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("February",
						db.Membertable.new_member(2, year, s.withMonth(2).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("March",
						db.Membertable.new_member(3, year, s.withMonth(3).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("April",
						db.Membertable.new_member(4, year, s.withMonth(4).lengthOfMonth())));
				series.getData().add(
						new XYChart.Data<>("May", db.Membertable.new_member(5, year, s.withMonth(5).lengthOfMonth())));
				series.getData().add(
						new XYChart.Data<>("June", db.Membertable.new_member(6, year, s.withMonth(6).lengthOfMonth())));
				series.getData().add(
						new XYChart.Data<>("July", db.Membertable.new_member(7, year, s.withMonth(7).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("August",
						db.Membertable.new_member(8, year, s.withMonth(8).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("September",
						db.Membertable.new_member(9, year, s.withMonth(9).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("October",
						db.Membertable.new_member(10, year, s.withMonth(10).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("November",
						db.Membertable.new_member(11, year, s.withMonth(11).lengthOfMonth())));
				series.getData().add(new XYChart.Data<>("Decomber",
						db.Membertable.new_member(12, year, s.withMonth(12).lengthOfMonth())));
	...
	void printReleasedAction(MouseEvent event) {
		...
		if (dateForData.getValue() != null && !txtfieldFinePerMinute.getText().isEmpty()
				&& !txtfieldFinePerDay.getText().isEmpty()) {
			if (completeCircFineDay.getFill() == Color.GREEN && completeCircFineMinute.getFill() == Color.GREEN) {
				Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "Are you sure to print the data?", ButtonType.YES,
						ButtonType.NO);
				alert.showAndWait();
				...
				if (alert.getResult() == ButtonType.YES) {
					try {
						...
						controller.addFineData(Integer.parseInt(txtfieldFinePerDay.getText()),
								Integer.parseInt(txtfieldFinePerMinute.getText()),
								String.valueOf(dateForData.getValue()));
