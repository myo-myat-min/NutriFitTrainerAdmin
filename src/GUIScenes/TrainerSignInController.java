...
public class TrainerSignInController implements Initializable {
	...
	void passAction(ActionEvent event) {
		...
		System.out.println(completeCircName.getFill());
		System.out.println(completeCircPass.getFill());

		finalSignInCheck(event);
	...
	public void initialSignInCheck() {
	...
	public void finalSignInCheck(ActionEvent event) {
		if (!txtFieldEmail.getText().isEmpty() && !pass.getText().isEmpty()) {
			if (completeCircName.getFill() == Color.GREEN && completeCircPass.getFill() == Color.GREEN) {
					...
					try (Connection con = ConnectDB.CreateConnection();
						...
						while (rs.next()) {
							...
							if (rs.getString("email").equals(txtFieldEmail.getText())
								...
								if (rs.getString("status").equals("On Job")) {
									...
									controller.addingData(rs.getString("name"), rs.getString("email"),
											rs.getString("password"), rs.getString("nrc"), rs.getString("phone_no"),
						...
						if (countForAlert > 0) {
							new Alert(Alert.AlertType.ERROR,
									"Please check your email or password again. Forgot password?", ButtonType.OK)
											.showAndWait();
						}
		...
		txtForgetPass.setOnMouseClicked((event) -> {
			if (ConnectDB.CreateConnection() != null) {
				...
				try (Connection con = ConnectDB.CreateConnection(); PreparedStatement ps = con.prepareStatement(sql);) {
					...
					while (rs.next()) {
						if (rs.getString("email").equals(txtFieldEmail.getText())
								&& !rs.getString("password").equals(pass.getText())) {
							// Decrypt password
							send("mstzsn01@gmail.com", "qazmlp123!", txtFieldEmail.getText(), "NutriFit - Password",
									rs.getString("password"));
							new Alert(AlertType.INFORMATION, "Password is sent to your email").showAndWait();
							countBothWrong = 0;
							countBothRight = 0;
							break;
						} else if (rs.getString("email").equals(txtFieldEmail.getText())
								&& rs.getString("password").equals(pass.getText())) {
							countBothRight++;
							countBothWrong = 0;
							break;
					...
					if (countBothWrong > 0) {
						new Alert(AlertType.ERROR, "Your email has not registered yet. Please check your email again.")
								.showAndWait();
					}
					if (countBothRight > 0) {
...
//        Plz insert wrong password SQL code in if condition
	...
	public void forgetPassHover() {
		Stop[] stop = { new Stop(0, Color.BLACK), new Stop(1, Color.web("#fb0")) };
